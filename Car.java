...
REMEMBER: CONSTRUCTOR -  has no return type and are as the same name as the class.
          ...
          METHODS -  has a return type everytime even if there is no data to be returned, void 
                     return type specifies no data to be returned. always return the correct type.
...
public class Car  //class name car is the blueprint for the objects to work.
{
    ...
    public Car(int speed,int doors) // a constructor,which has the same name of the class and does not have a return type.
    { // Its a parameterised constructor having two integer type arguments for passing the value.
    ...
    public int howfastcanitgo(int total) // a parameterised Method of return type int with one argument.
    {
        int newspeed = maxspeed + total; //adding new passed value to the maxspeed to get new speed.
